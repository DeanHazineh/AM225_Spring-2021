...
// *********************************************************************************************************************** 
// ************************* Constructor and Driver Calls (e.g. initialize and solve)********
// ***********************************************************************************************************************
...
fluid_2d::fluid_2d(const int m_,const int n_,const bool x_prd_,
    ...
    cTripletArray(new double[3*m*n]){}
...
void fluid_2d::solve(double duration,int frames) {
    ...
    for(int k=1;k<=frames;k++) {
        ...
        write_files(k+f_num);
		  ...
		  // output the color png -- also can consider just saving to png at end for speed
		  char *bufc=((char*) buf);
	     sprintf(bufc,"%s/%s_%d.png",filename,"colors",k+f_num);
	     outputColorField(bufc);
    ...
    /* // Option to consider just saving final frame
    char *bufc=((char*) buf);
    sprintf(bufc,"%s/%s_End.png",filename,"colors");
    outputColorField(bufc);*/
...
// Restores the velocity and pressure field back to zero 
// Depending on when it is used, 
// it is not totally realistic because one cant do that in real life
// but it could make prettier pictures 
void fluid_2d::resetVelocityPressureField(){
...
/** Initializes the simulation fields -- Rycroft function*/
...
/** alternative initialization function for velocity-- this produces centered twist effect */
void fluid_2d::init_fields_CustomFunc(){
	// Loop over the primary grid and set the velocity and pressure
	#pragma omp parallel for
	for(int j=0;j<n;j++) {
		double y=ay+dy*(j+0.5),yy=y+0.5;
		...
		for(int i=0;i<m;i++){
			double x=ax+dx*(i+0.5),xx=x+0.5;
			fp->u=(x*x - y*y)*sin((x+y)/2.)/(x*x + y*y);
			fp->v=(x*x - y*y)*sin((x+y)/2.)/(x*x + y*y);;
			(fp++)->p=0;
		}
		fp->p=0;
	}		
...
// Has the effect of taking your finger and sliding the water from top to bottom
void fluid_2d::renew_VelocityField1(double xcenter, double ycenter, double circRad){
	#pragma omp parallel for
	for(int j=0;j<n;j++) {
		double y= ycenter-(ay+dy*(j+0.5));
		field *fp=fm+ml*j;
   	for(int i=0;i<m;i++){
      	double x=xcenter-(ax+dx*(i+0.5));
   		if( sqrt(x*x + y*y)<circRad ){
   			fp->u= pow(sin(x+y),2);
            fp->v= pow(cos(x+y),2);
            (fp++)->p=0;
   		}else{
...
// Has the effect of taking your finger and sliding the water from top to bottom
void fluid_2d::renew_VelocityField2(double xcenter, double ycenter, double circRad){
	#pragma omp parallel for
	for(int j=0;j<n;j++) {
		double y= ycenter-(ay+dy*(j+0.5));
		field *fp=fm+ml*j;
   	for(int i=0;i<m;i++){
      	double x=xcenter-(ax+dx*(i+0.5));
   		if( sqrt(x*x + y*y)<circRad ){
   			fp->u= -1.*pow(sin(x+y),2);
            fp->v= -1.*pow(cos(x+y),2);
            (fp++)->p=0;
   		}else{
   			fp++;
   		}   	
...
// ************************* Set Color Fields ********
// ***********************************************************************************************************************
// Envision using nebula color scheme for space like pallete - 
//http://paperheartdesign.com/blog/color-palette-awesome-space

/** Initialize the color field  (similar manner as initializing the velocity field)*/
void fluid_2d::init_Color(){
	// loop over primary grid and initialize R,G,B
   #pragma omp parallel for
	for(int j=0;j<n;j++) {
      ...
      for(int i=0;i<m;i++) {
      	...
      	if( fmod(floor(6.*x+6.)+floor(6.*y+6.),2.0)==0. ){
      	// code modified from part a to part b -- new conditions
      		fp->R = 33/256.;
      		fp->G = 5/256.;
      		(fp++)->B = 53/256.;
      	}else{
...
void fluid_2d::AddCircle_colorfield(double xcenter, double ycenter, double circRad, double rcoef, double bcoef, double gcoef){
	#pragma omp parallel for
	for(int j=0;j<n;j++) {
		double y= ycenter-(ay+dy*(j+0.5)); 
		field *fp=fm+ml*j;
...
void fluid_2d::set_boundaries() {
    ...
    if(x_prd) {
        ...
        for(field *fp=fm,*fe=fm+n*ml;fp<fe;fp+=ml) {
            ...
            // add non-periodic color ghost nodes-- reflection 
            fp[-2].no_slipColor(fp[1]);
            fp[-1].no_slipColor(*fp);
            fp[m].no_slipColor(fp[m-1]);
            fp[m+1].no_slipColor(fp[m-2]);
        }
    }
    ...
    if(y_prd) {
        ...
        for(field *fp=fm-2,*fe=fm+m+2;fp<fe;fp++) {
            ...
            // add non-periodic color ghost nodes
            fp[-tl].no_slipColor(fp[ml]);
            fp[-ml].no_slipColor(*fp);
            fp[g].no_slipColor(fp[g-ml]);
            fp[g+ml].no_slipColor(fp[g-tl]);
        }
    }
}
...
// *********************************************************************************************************************** 
// ************************* Utility Functions e.g prints ********
// ***********************************************************************************************************************
void fluid_2d::outputColorField(const char *fname){
	// task is to loop over the grid points and to store the color triplets into double structures
	// then we may use rycrofts write_png code to save snapshot of current state to png file
	double* cp = cTripletArray;
	for(int j=0;j<n;j++) {
		field *fp=fm+ml*j;
      for(int i=0;i<m;i++) {
   		*(cp++)=fp->R;
   ...
   write_png(fname,m,n,cTripletArray, 0.,1.);
}
...
void fluid_2d::step_forward(double dt) {
	...
	/** Let us now take the current u and v state and
	derive the updated state for the color field. After doing this, u and v will be solved to get
	the u and v to be used at the next time step
	-- For efficiency, we should be able to do it in the same loop as that 
	used to compute the intermediate velocity field*/
...
#pragma omp parallel for
    for(j=0;j<n;j++) for(int i=0;i<m;i++) {
        ...
        /** Color field update */
        // compute the spatial derivatives of the color field at each grid point
        // hx and hy is already the correct prefactor to be used in euler step for time
        double rxd, ryd, gxd, gyd, bxd, byd;
        uc>0?color_eno2(rxd, gxd, bxd, hx, fp[1],f,fp[-1],fp[-2])
        	:color_eno2(rxd, gxd, bxd, -hx,fp[-1],f,fp[1],fp[2]);
        vc>0?color_eno2(ryd, gyd, byd, hy, fp[ml],f,fp[-ml],fp[-2*ml])
        	:color_eno2(ryd, gyd, byd, -hy,fp[-ml],f,fp[ml],fp[2*ml]);
		 
        
		  // Implement Euler update step for color fields
		  // derivatives of color already contain delta t factor
		  f.R = f.R - uc*rxd - vc*ryd;
		  f.G = f.G - uc*gxd - vc*gyd;
		  f.B = f.B - uc*bxd - vc*byd;
    }
...
/** Calculated derivatives of the scalar color field rgb using EN02
similar to the above implementation for velocity*/
inline void fluid_2d::color_eno2(double &rd, double &gd, double &bd,double hs,field &f0,field &f1,field &f2,field &f3){
